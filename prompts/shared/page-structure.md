...
- 预览缩放：`--preview-scale` + `.page-wrap { transform: scale(...); margin-right: calc(1080px * (var(--preview-scale) - 1)); }`；`margin-bottom` 须随页高在 `</body>` 前用内联 script 计算（参考 template.html），勿依赖宿主页面事后改 DOM
